...
const VIDEO_PATH = "./antal-commercial/public/script2_podcast.mp4";
const AUDIO_PATH = "./antal-commercial/public/voiceover.mp3";
const OUTPUT_PATH = "./antal-commercial/public/final_podcast_synced.mp4";
...
async function main() {
  ...
  // Upload video
  console.log(`[1/4] Uploading video to fal storage…`);
  if (!fs.existsSync(VIDEO_PATH)) {
    console.error(`Video not found: ${VIDEO_PATH}`);
    process.exit(1);
  }
  const videoBuf = fs.readFileSync(VIDEO_PATH);
  const videoFile = new File([videoBuf], "script2_podcast.mp4", { type: "video/mp4" });
  const videoUrl = await fal.storage.upload(videoFile);
  console.log(`      uploaded: ${videoUrl}`);

  // Upload audio
  console.log(`[2/4] Uploading audio to fal storage…`);
  ...
  // Call sync-lipsync
  console.log(`[3/4] Submitting sync-lipsync: ${MODEL}`);
